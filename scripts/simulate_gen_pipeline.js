...
async function simulatePipeline() {
    ...
    const generator = new TopicContentGenerator(topicId, (state) => {
        ...
        if (state.status === 'error') {
            console.error(`🔴 ESTADO: Error - ${JSON.stringify(state.steps.find(s => s.status === 'error'))}`);
        } else if (step) {
    ...
    try {
        ...
        console.log(`Total Palabras: ${result.sections.reduce((acc, s) => acc + (s.content?.text?.split(/\s+/).length || 0), 0)}`);
