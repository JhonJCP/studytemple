...
export function SourceReference({ text, source, inline = true }: SourceReferenceProps) {
    ...
    const showInlineTag =
        inline &&
        // Evitar duplicar etiquetas cuando el propio texto ya contiene la referencia legal
        !/\b(art\.?|art[ií]culo)\b/i.test(text);
                    ...
                    >
                        <div className="flex items-center justify-between mb-4">
                            <div>
                                ...
                                {source.article && (
                                    <p className="text-sm text-gray-600 dark:text-gray-300">{source.article}</p>
                                )}
                            </div>
                            <button
                                onClick={() => setShowModal(false)}
                        ...
                        <div className="mt-4 text-xs text-gray-500">ChunkID: {source.chunkId || "N/A"}</div>
                    </div>
                </div>
            )}
        </>
